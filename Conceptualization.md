...
* **Adaptive Trading System (System)**: 유전 알고리즘 기반 최적화 엔진과 실시간 매매 실행 로직이 통합된 본 프로젝트의 핵심 시스템입니다.
* **User (Actor)**: API Key, 심볼, 레버리지 등 초기 설정값을 입력하고 시스템의 성과 리포트를 확인하는 주체입니다.
* **Binance API (Actor)**: 실시간/과거 시세 데이터를 제공하고, 시스템의 주문 명령을 받아 체결 결과를 반환하는 외부 거래소 플랫폼입니다.
* **TA Library (Actor)**: `pandas_ta` 등 오픈소스 라이브러리를 통해 시세 데이터를 분석하여 지표(ADX, Slope 등)를 제공합니다.
